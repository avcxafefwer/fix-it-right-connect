...
-- Generate time slots for the next 30 days (9 AM to 5 PM, Monday to Saturday)
INSERT INTO public.time_slots (date, start_time, end_time)
SELECT 
  date_series::date,
  time_slot::time,
  (time_slot + interval '2 hours')::time
FROM 
  generate_series(CURRENT_DATE + 1, CURRENT_DATE + 30, '1 day'::interval) AS date_series,
  generate_series('09:00'::time, '15:00'::time, '2 hours'::interval) AS time_slot
WHERE 
  EXTRACT(DOW FROM date_series) BETWEEN 1 AND 6; -- Monday to Saturday
